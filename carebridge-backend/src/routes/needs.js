...
const { runMatchingEngine } = require('./matching');
...
router.get('/', async (req, res) => {
  ...
  query += ' ORDER BY n.priority_score DESC, n.created_at DESC';
...
router.post('/', authenticate, authorise('facility', 'admin'), async (req, res) => {
  ...
  try {
    ...
    // Fire-and-forget: run matching engine in the background
    // Don't await — respond to the client immediately
    runMatchingEngine(newNeed.id)
      .then((match) => {
        if (match && match.priority_score) {
          pool.query(
            'UPDATE needs SET priority_score = $1 WHERE id = $2',
            [match.priority_score, newNeed.id]
          ).catch(console.error);
        }
      })
      .catch((err) => console.error('Matching engine error:', err.message));
