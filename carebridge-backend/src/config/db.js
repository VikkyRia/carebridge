...
const initDB = async () => {
  ...
  // Add priority_score column if upgrading from old schema
  await pool.query(`
    ALTER TABLE needs ADD COLUMN IF NOT EXISTS priority_score NUMERIC(5,2) DEFAULT 0;
  `).catch(() => {}); // ignore if already exists

  // Add UNIQUE constraint to transaction_ref if upgrading
  await pool.query(`
    DO $$ BEGIN
      ALTER TABLE donations ADD CONSTRAINT donations_transaction_ref_key UNIQUE (transaction_ref);
